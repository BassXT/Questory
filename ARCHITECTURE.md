...
Avatar-Items sind ein globaler Katalog in der Datenbank. Level-Freischaltungen werden beim Lesen aus `AvatarItem.requiredLevel` berechnet; `ChildAvatarItem` ist fuer manuelle, Event- oder Spezial-Unlocks vorbereitet. Die aktive Werkstatt kombiniert genau ein vollstaendiges Kinderfiguren-Preset (`character`) mit einem optionalen Begleittier (`pet`). Beide werden als normalisierte, transparente PNGs auf einer festen Buehne dargestellt, sodass keine fehleranfaellige Laufzeitkomposition von Haaren, Kleidung und Koerperteilen mehr noetig ist. Die Auswahl liegt pro Kind in `ChildAvatarLoadout.equippedItems` als validiertes JSON. Alte modulare Item-Keys und gespeicherte Slot-Daten bleiben inaktiv in der Datenbank erhalten und werden von der aktiven API ignoriert.
