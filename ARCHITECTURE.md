...
- Der Avatar-Builder ist in `apps/frontend/src/avatar-builder.tsx` gekapselt; die Zeichenengine liegt getrennt in `apps/frontend/src/soft-avatar-renderer.tsx`, die Key-zu-Art-Zuordnung in `apps/frontend/src/illustrated-avatar-assets.ts`. Die Komposition nutzt ein gemeinsames `360x520`-SVG-Koordinatensystem fuer stabile Kopf-, Koerper-, Hand- und Fussanker. Gesicht, Haut, Arme, Beine und Szenen werden als skalierbare SVG-Basis gezeichnet. Komplexe Haare, Kleidung, Schuhe, Huete, Brillen, Rucksaecke, Waffen, Kamera und Tiere werden als transparente, verlustfreie WebP-Layer aus `apps/frontend/public/avatar-art/v1` eingebettet. Die Assets werden mit `scripts/extract-avatar-art.py` reproduzierbar aus der unter `docs/design` versionierten, freigegebenen Stilreferenz erzeugt. So bleibt Questory self-hosted und API-unabhaengig, waehrend die sichtbaren Teile der hochwertigen Abenteuerillustration entsprechen.
