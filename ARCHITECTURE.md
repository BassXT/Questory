...
- Der Avatar-Builder ist in `apps/frontend/src/avatar-builder.tsx` gekapselt; die Zeichenengine liegt getrennt in `apps/frontend/src/soft-avatar-renderer.tsx`, die Key-zu-Art-Zuordnung samt Attachment-Metadaten in `apps/frontend/src/illustrated-avatar-assets.ts`. Die statische Paper-Doll-Komposition nutzt ein gemeinsames `360x520`-SVG-Koordinatensystem und benannte Anker fuer Kopf, Gesicht, Schultern, Huefte, Fuesse, Ruecken und Bodenobjekte. Jedes WebP besitzt eigene Breite, Hoehe, Ursprung und optionale Offsets relativ zu seinem Anker; dadurch werden unterschiedlich zugeschnittene Katalogmotive nicht mehr mit einer generischen Slot-Geometrie uebereinandergelegt. Eine explizite Zeichenreihenfolge trennt Haar-Rueckseite, Rueckenobjekte, neutrales Koerper-Rig, Kleidung, Haende, Kopf/Gesicht, Haar-Vorderseite, Accessoires und Begleiter.
- Das Koerper-Rig ist strukturell notwendig, bleibt aber weitgehend unsichtbar: Es liefert nur Hals, verdeckte Gliedmassen und vor der Kleidung liegende Haende. Ein zusaetzliches sichtbares SVG-Outfit unter den WebP-Kleidungsstuecken wird nicht mehr gerendert. Komplexe Haare, Kleidung, Schuhe, Huete, Brillen, Rucksaecke, Waffen, Kamera und Tiere liegen als transparente, verlustfreie WebP-Layer unter `apps/frontend/public/avatar-art/v1`. `scripts/extract-avatar-art.py` erzeugt sie reproduzierbar aus der unter `docs/design` versionierten Stilreferenz und entfernt bei Haarmotiven auch den leeren Modellkopf, Ohren und Hals des Katalogbildes.
- Die Rig-Entscheidung folgt bewaehrten 2D-Kompositionsmustern: Spine trennt Skeleton, Slots, Attachments, Skins und Draw Order; PixiJS trennt Scene-Graph-Transformationen, Anchor/Pivot und Render Layers. Questory bildet diese Prinzipien ohne zusaetzliche Runtime-Abhaengigkeit im vorhandenen SVG-Renderer ab, weil die Avatare aktuell statisch sind. Referenzen: [Spine Skins](https://eu.esotericsoftware.com/spine-skins), [Spine Basic Concepts](https://en.esotericsoftware.com/spine-basic-concepts), [PixiJS Scene Objects](https://pixijs.com/8.x/guides/components/scene-objects), [PixiJS Render Layers](https://pixijs.com/8.x/guides/concepts/render-layers).
