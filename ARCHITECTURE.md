...
- Der Avatar-Builder ist in `apps/frontend/src/avatar-builder.tsx` gekapselt; die aktive Zeichenengine liegt getrennt in `apps/frontend/src/kenney-avatar-renderer.tsx`, die Key-zu-Art-Zuordnung fuer ergaenzende Picker-Vorschauen in `apps/frontend/src/illustrated-avatar-assets.ts`. Der Renderer normalisiert einen kuratierten CC0-Teilesatz aus `Kenney Modular Characters` auf ein festes, kindlich proportioniertes `360x520`-Questory-Rig. Kopf, Hals, Arme, Haende, Oberteil, Unterteil, Beine und Schuhe besitzen feste Koordinaten und eine feste Zeichenreihenfolge; Farben werden ueber SVG-Masken angewendet. Dadurch bleiben freie Haar-/Augenfarben sowie bestehende Item-Keys und Loadouts kompatibel, ohne dass Kleidung und Koerper frei gegeneinander verschoben werden.
...
- Die freigegebene Zielrichtung ist ein statischer 2D-Paper-Doll-Builder. Akzeptiert werden echte transparente Ganzkoerper-Layer oder ein kompatibles Einzelteil-Rig, das ueber feste Anker in die gemeinsame Questory-Leinwand normalisiert wird. Der verbindliche Import- und Ebenenvertrag steht in `docs/design/AVATAR_ASSET_CONTRACT.md`. Geschlechter duerfen getrennte kompatible Layerkataloge besitzen; Grundfarben bleiben frei, waehrend besondere Stile ueber Level freigeschaltet werden.
- Avatarquellen muessen kostenlos und fuer den privaten Self-hosted-Betrieb rechtlich eindeutig nutzbar sein; CC0 wird bevorzugt. Der erste Produktions-Referenzsatz stammt aus `Kenney Modular Characters`, liegt mit Lizenz und Quellenhinweis unter `apps/frontend/public/avatar-kenney/v1` und wird als Einzelteil-Rig mit festen Questory-Ankern verwendet. Die Zielarchitektur folgt damit weiterhin bewaehrten Paper-Doll-Mustern, ohne eine zusaetzliche Grafik-Runtime einzufuehren.
