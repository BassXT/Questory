...
- Der Avatar-Builder ist in `apps/frontend/src/avatar-builder.tsx` gekapselt. Die visuelle Darstellung nutzt einen Hybrid-Renderer: Kopf, Haare und Gesicht kommen ueber `@dicebear/core` mit dem einzelnen Stil `@dicebear/toon-head`, Koerper, Oberteil, Hose, Schuhe, Hintergrund und Gadgets rendert Questory als eigene SVG-Layer. Die externe DiceBear-HTTP-API wird nicht genutzt; dadurch bleibt der Renderer self-hosted und reproduzierbar, ohne die komplette DiceBear-Collection zu buendeln.
