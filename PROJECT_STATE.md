...
- Die visuelle Avatar-Darstellung nutzt den Questory-eigenen `SoftAdventureAvatarGraphic`-Renderer. Das gemeinsame `360x520`-SVG-Koordinatensystem bleibt die Kompositionsbuehne, verwendet aber fuer komplexe Illustrationen transparente, verlustfreie WebP-Layer aus der verbindlichen Stilreferenz. Gesicht, Haut, Arme, Beine und Szenen bleiben als skalierbare SVG-Basis erhalten. Die Hybridloesung liefert deutlich reichere Haare, Kleidung und Sammelobjekte, ohne externe Avatar-API und ohne die bestehenden Slot-/Loadout-Vertraege zu veraendern.
