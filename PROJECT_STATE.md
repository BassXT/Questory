...
Questory besitzt einen auf dem Docker-LXC laufenden MVP-Stack aus PostgreSQL, NestJS und React/Vite. Authentifizierung, Familien- und Kinderverwaltung, Quest-Erstellung und -Zuweisung, Abschlussfreigaben mit XP und Muenzen, Reward-Verwaltung, anfragepflichtiger Shop, Einloesungsverwaltung, Statistiken und Kinder-Dashboard sind implementiert und wurden auf dem LXC getestet.

Das Frontend trennt Eltern- und Kinderansicht klar. Eltern verwalten Kinder, Quests, Belohnungen, Zuweisungen und Freigaben; Kinder sehen nur ihren Status, Avatar, zugewiesene Quests und Shop-Inhalte. Reward-Anfragen reservieren Muenzen und geben sie bei Ablehnung oder Storno zurueck. CI, Portainer-Redeploy-Automation, Backup-Script und Dry-Run-first-Testdatenbereinigung sind vorhanden. Docker ist lokal auf Windows weiterhin nicht im PATH verfuegbar.
...
- Acht vollstaendige Kinderfiguren erzeugt und freigestellt: zwei Alltagslooks, Smiley, Stern, Entdeckerin, Ritter, Astronautin und Zauberin.
- Acht separat waehlbare Begleittiere erzeugt und freigestellt: Katze, Hund, Hase, Fuchs, Sonnenfell, Tiger, Elefant und Walddrache.
...
- Historische Export-Pipeline fuer den verworfenen Einzelteil-Renderer dokumentiert und zusammen mit den nicht mehr genutzten Zwischenassets entfernt.
...
- Questory wird aktuell ausschliesslich privat fuer die eigene Familie betrieben. Externe Laufzeitdienste bleiben optional; Familien-, Kinder- und Fortschrittsdaten verbleiben im self-hosted Stack.
...
- Neue Figuren- und Tierassets muessen fuer den privaten Self-hosted-Betrieb rechtlich eindeutig nutzbar sein, denselben Stil einhalten und vor dem Import auf Transparenz, Beschnitt, Bodenlinie und mobile Lesbarkeit geprueft werden.
